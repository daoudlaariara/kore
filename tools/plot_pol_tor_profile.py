...
'''
Script to plot the radial profile of a given poloidal/toroidal component

nsol   : solution number
radius : radial location
field  : 'u' or 'b' for flow or magnetic field
comp : 'T' or 'P' for Toroidal or Poloidal component
l : degree of component
'''
...
if comp == 'T' : 
    plt.plot(r[sortr],abs(Tlr[il,sortr]),'o-',ms=3,lw=1,label=r'Toroidal')
elif comp == 'P' : 
    plt.plot(r[sortr],abs(Plr[il,sortr]),'o-',ms=3,lw=1,label=r'Poloidal')
else : 
    print("Component must be either T or P")

plt.xlabel(r'Radius $r$',size=14)
plt.ylabel(f'Component amplitude',size=14)
plt.title("${}{}{}$".format(comp, l, m))
plt.legend()
