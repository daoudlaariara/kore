...
'''
Script to plot the equatorial cut of a solution field
Use as:

python3 path/to/plot_equat_field.py nsol theta0 theta1 coord field opt

nsol   : solution number
phi0   : starting azimuthal angle
phi1   : ending azimuthal angle
...
phi = np.linspace(phi0*np.pi/180, phi1*np.pi/180, nphi+2)
phi = phi[1:-1]
...
ur = np.zeros((nr) * nphi,dtype=complex)
utheta = np.zeros((nr) * nphi,dtype=complex)
uphi = np.zeros((nr) * nphi,dtype=complex)

if coord == 'cyl':
    uz = np.zeros((nr) * nphi,dtype=complex)
    us = np.zeros((nr) * nphi,dtype=complex)
...
for kp in range(nphi):
    ylm = np.r_[Ylm_full(lmax, m, tta, phi[kp]),0]
    for kr in range(0,nr):
        x[k] = r[kr] * np.sin(tta) * np.cos(phi[kp])
        y[k] = r[kr] * np.sin(tta) * np.sin(phi[kp])

        ur[k] = np.dot( Qlr[:,kr], ylm[idP:plx:2] )

        tmp1 = np.dot(           -(llpol+1) * Slr[:,kr]/np.tan(tta), ylm[idP:plx:2]     )
        tmp2 = np.dot( clm[idP+1:plx+1:2,0] * Slr[:,kr]/np.sin(tta), ylm[idP+1:plx+1:2] )
        tmp3 = np.dot(                 1j*m * Tlr[:,kr]/np.sin(tta), ylm[idT:tlx:2]     )
        utheta[k] = tmp1+tmp2+tmp3

        tmp1 = np.dot(             (lltor+1) * Tlr[:,kr]/np.tan(tta), ylm[idT:tlx:2]     )
        tmp2 = np.dot( -clm[idT+1:tlx+1:2,0] * Tlr[:,kr]/np.sin(tta), ylm[idT+1:tlx+1:2] )
        tmp3 = np.dot(                  1j*m * Slr[:,kr]/np.sin(tta), ylm[idP:plx:2]     )
        uphi[k] = tmp1+tmp2+tmp3

        if coord == 'cyl':
            uz[k] = ur[k] * np.cos(tta) - utheta[k] * np.sin(tta)
            us[k] = ur[k] * np.sin(tta) + utheta[k] * np.cos(tta)
...
ax1.plot(r[0]*np.sin(phi), r[0]*np.cos(phi),'k',lw=0.4)
ax1.plot(r[-1]*np.sin(phi), r[-1]*np.cos(phi),'k',lw=0.4)
...
ax2.plot(r[0]*np.sin(phi), r[0]*np.cos(phi),'k',lw=0.4)
ax2.plot(r[-1]*np.sin(phi), r[-1]*np.cos(phi),'k',lw=0.4)
...
ax3.plot(r[0]*np.sin(phi), r[0]*np.cos(phi),'k',lw=0.4)
ax3.plot(r[-1]*np.sin(phi), r[-1]*np.cos(phi),'k',lw=0.4)
